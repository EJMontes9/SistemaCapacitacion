...
@section('title', 'Editar Plantilla - ' . $template->course->title)

@section('content_header')
    <h1 class="font-weight-bold">Editar Plantilla: {{ $template->course->title }}</h1>
...
@section('content')
    ...
    <form action="{{ route('admin.certificates.update', $template) }}" method="POST" id="templateForm">
        @csrf @method('PUT')
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" id="min_attendance" name="min_attendance"
                                        value="{{ old('min_attendance', $template->min_attendance ?: 80) }}" min="0" max="100">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" id="min_grade" name="min_grade"
                                        value="{{ old('min_grade', $template->min_grade ?: 70) }}" min="0" max="100">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <small class="form-text text-muted mb-2">
                                Variables: <code>@studentName</code>, <code>@courseName</code>, <code>@completionDate</code>, <code>@certificateCode</code>
                            </small>
                            ...
                            <textarea class="form-control" id="body_html" name="body_html" rows="25" style="font-family: monospace; font-size: 13px; tab-size: 2;">{{ old('body_html', $template->body_html) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        document.getElementById('previewBtn').addEventListener('click', function() {
            ...
            fetch('{{ route('admin.certificates.preview') }}', {
                ...
                body: JSON.stringify({ body_html: html, student_name: 'María García López', course_name: '{{ $template->course->title }}' })
            })
